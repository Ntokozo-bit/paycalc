...
(function () {
    ...
    const el = {
        ...
        qa_workTimeFields: document.getElementById("qa_workTimeFields"),
        qa_breakControl: document.getElementById("qa_breakControl"),
        ...
        qa_applyOtControl: document.getElementById("qa_applyOtControl"),
        ...
        ed_workTimeFields: document.getElementById("ed_workTimeFields"),
        ...
        ed_applyOtControl: document.getElementById("ed_applyOtControl"),
    ...
    function normalizeSettings(raw) {
        ...
        const template = Array.isArray(source.weekTemplate) ? source.weekTemplate : DEFAULT_SETTINGS.weekTemplate;
        return {
            hourly: clamp(source.hourly ?? DEFAULT_SETTINGS.hourly, 0, 1e9),
            otThreshold: clamp(source.otThreshold ?? DEFAULT_SETTINGS.otThreshold, 0, 24),
            otMultiplier: clamp(source.otMultiplier ?? DEFAULT_SETTINGS.otMultiplier, 1, 10),
            sundayOrdinaryMultiplier: clamp(
                source.sundayOrdinaryMultiplier ?? DEFAULT_SETTINGS.sundayOrdinaryMultiplier,
                1,
                10
            ),
            ...
            weekTemplate: Array.from({ length: 7 }, (_, index) => {
                ...
                return {
                    start: parseTime(day.start) === null ? "" : day.start,
                    end: parseTime(day.end) === null ? "" : day.end
                };
            })
        };
    }
    ...
    function persistJson(key, value) {
        try {
            ...
            if (!storageFailureShown) {
                ...
                window.alert("WorkPay could not save on this device. Free some browser storage, then download a backup before closing the app.");
            }
            return false;
        }
    }
    ...
    function saveSettings() {
        return persistJson(STORE.SETTINGS, settings);
    }
    ...
    function scheduledHoursForDate(value) {
        const date = toDate(value);
        if (!date) return 0;
        const template = settings.weekTemplate?.[date.getDay()] || {};
        const start = parseTime(template.start);
        const endValue = parseTime(template.end);
        if (start === null || endValue === null) return 0;
    ...
    function isUsualSunday(value) {
        ...
        const template = settings.weekTemplate?.[0] || {};
        return parseTime(template.start) !== null && parseTime(template.end) !== null;
    }

    function resolveRates(row) {
        const usualSunday = typeof row.sundayWasUsual === "boolean"
            ? row.sundayWasUsual
            : isUsualSunday(row.dateISO);
        const configuredSundayMultiplier = usualSunday
            ? settings.sundayOrdinaryMultiplier
            : settings.sundayMultiplier;
    ...
    function calcRow(row) {
        ...
        // number for the public-holiday daily wage instead of guessing from a
        // start/end schedule that may include different break arrangements.
        const ordinaryDailyHours = frozenDailyHours ?? otTh;
    ...
    function calcBasicRow(row) {
        ...
        const basicHours = (calc.paidOff || calc.holidayNotWorkedPaid)
            ? calc.paidHours
            : (calc.specialType === "holiday" && calc.holidayWorked && calc.holidayOrdinaryWorkday)
                ? calc.ordinaryDailyHours
    ...
    function freezeRowForHistory(row) {
        ...
        return {
            ...row,
            sundayWasUsual: !!rates.usualSunday,
            overrides: {
                useGlobal: false,
    ...
    function syncSettingsToForm() {
        ...
        el.s_otThreshold.value = settings.otThreshold ?? 9;
    ...
    function autoFillCycle() {
        ...
        for (const day of datesBetween(range.start, range.end)) {
            ...
            const publicHoliday = isAutoHoliday(day);
            generated.push(entryFromValues({
                date: ymd(day),
                start: publicHoliday ? "" : (template.start || ""),
        ...
        if (!generated.length) {
            window.alert("This cycle is already filled. Your saved days were left unchanged.");
            return;
        }

        const ok = window.confirm(
            `Add ${generated.length} missing scheduled day${generated.length === 1 ? "" : "s"}? Existing and custom days will be kept.`
        );
        if (!ok) return;
    ...
    function syncSpecialDayControls(scope, prefillWorkedTimes = false) {
        ...
        holidayField.checked = holiday;
        holidayField.disabled = automaticHoliday;
        ...
        const fields = isEdit
            ? [el.ed_start, el.ed_end, el.ed_break]
            : [el.qa_start, el.qa_end, el.qa_break];
        fields.forEach(field => { field.disabled = disableWorkFields; });
        workTimeFields.hidden = disableWorkFields;
        if (!isEdit) el.qa_breakControl.hidden = disableWorkFields;
    ...
    function buildPayDetailText(calc) {
        ...
        if (calc.specialType === "holiday" && calc.holidayWorked) {
            ...
            return `Double-day minimum ${(calc.ordinaryDailyHours * calc.multiplier).toFixed(2)}h equivalent · worked ${calc.specialH.toFixed(2)}h`;
        }
    ...
    function requestDateEdit(dateStr) {
        document.dispatchEvent(new CustomEvent("workpay:edit-date", {
            detail: { date: dateStr }
        }));
    }
    ...
    function renderSelectedDay(range, rows) {
        ...
        if (editable) {
            ...
            const actions = make("div", "selected-actions");
            const editPast = make("button", "btn-primary", "Edit Past Day");
            editPast.type = "button";
            editPast.addEventListener("click", () => requestDateEdit(selectedDate));
            actions.appendChild(editPast);
            el.selectedDayPanel.appendChild(actions);
            el.selectedDayPanel.appendChild(make("p", "helper", "Editing keeps the original saved rates unless you choose different per-day settings."));
        } else {
            el.selectedDayPanel.appendChild(make("p", "helper", "No saved entry exists for this older date."));
        }
    }
    ...
    function buildCard(row, editable = true) {
        ...
        if (editable) {
            ...
            node.querySelector(".edit").textContent = "Edit Past Day";
            node.querySelector(".edit").addEventListener("click", () => requestDateEdit(ymd(row.dateISO)));
    ...
    function render() {
        ...
        renderSelectedDay(range, rows);
        renderEntries();
        document.dispatchEvent(new CustomEvent("workpay:rendered"));
    }
    ...
    document.addEventListener("workpay:data-changed", () => {
